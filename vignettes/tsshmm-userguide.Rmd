...
1. Create the model object with the initial parameters shown in Figure
   \ref{fig:hmm} using `new("TSSHMM")`,
2. Encode the read coverage counts into the 3 categories of discrete emissions
   observed --- no signal, enriched, and depleted --- using `create_batches()`,
   and then
3. Feeding the batches into the model for training using `train()`.  Because
   our subsampled dataset has only a small amount of data for the number of
   parameters to train, we loop the `train()` function 30 times for the
   parameters to converge and collect the `DataFrame` of the parameter changes
   to visualize in a later section.

```{r Train GRO-cap, dependson = "Inspect core2014 data", cache = params$cache}
## Initialize the model.
(model_grocap <- new("TSSHMM"))

## Encode the signal and background into batches that the model can understand.
system.time(
    batches_grocap <- create_batches(signal, bg_grocap)
)

## Train the model 30 times to converge parameters on this small batch dataset.
system.time(
    log_grocap <- train(model_grocap, batches_grocap)
)
...
1. Pass the argument `bg_proseq = TRUE` when initializing the model to add the
...
2. Use the GRO-seq data stored in `bg_groseq` instead of `bg_grocap` to
   generate the randomized training batches.

```{r Train GRO-seq, dependson = "Inspect core2014 data", cache = params$cache}
(model_groseq <- new("TSSHMM", bg_proseq = TRUE))

system.time(
    batches_groseq <- create_batches(signal, bg_groseq)
)

system.time(
    log_groseq <- train(model_groseq, batches_groseq)
)
...
## Big data training and visualization convergence

If there is only one batch in the data, then `train()` uses a special case
algorithm where it runs until convergence.  Such is the case with our subset
example data.  However for a real world mammalian genome dataset, it's not
possible to fit all the data at once into dense matrices for running the
forward-backward algorithm; therefore it becomes necessary to eyeball the
training history to decide whether additional `train()` runs across the full
dataset are necessary.  One can log and visualize the parameter updates as
follows:

```{r Big data training, dependson = "Inspect core2014 data", cache = params$cache}
library(futile.logger) # flog.threshold, INFO, ERROR

## Enable verbose progress from create_batches() and train() because they can
## take a long time to run on on big data sets.
flog.threshold(INFO)

system.time(
    ## Generate multiple batches by lowering the row cut-off to 150.
    batches_big <- create_batches(signal, bg_grocap, nrow = 150)
)
...
system.time({
    i <- 1
    n_iter <- 4
    message_iter <- function()
        message(sprintf("*** Looping entire dataset [%d/%d]", i, n_iter))
    message_iter()
    log <- train(model, batches_big)
    for (i in 2:n_iter) {
        message_iter()
        to_append <- train(model, batches_big)
        ## The first row contains the pre-training parameters; remove it when
        ## appending.
        log <- rbind(log, to_append[-1, ])
    }
})
...
```{r Plot training parameters, dependson = "Big data training", cache = params$cache, message = FALSE}
library(dplyr)   # as_tibble, mutate, row_number, %>%
library(tidyr)   # gather
library(stringr) # str_length

df_long <- function(df, bg)
    df %>%
        as_tibble() %>%
        mutate(i = row_number() - 1,
               samples = cumsum(samples)) %>%
        gather(key = "parameter", value = "probability", -i, -samples) %>%
        mutate(type = ifelse(str_length(parameter) > 5,
                             "emission", "transition"))
...
ggplot(df, aes(samples, probability, color = parameter, group = parameter)) +
    geom_line() +
    geom_label_repel(data = dplyr::filter(df, i == ceiling(max(i) / 2)),
                     segment.color = NA,
                     aes(label = parameter)) +
    facet_wrap(~ type) +
    guides(color = "none", label = "none") +
    labs(title = paste("Model training after", n_iter, " iterations"),
         x = "Number of samples used for training",
         y = "Parameter probability value")
```

The graph above that our parameters look like they are starting to converge
after our insufficient `r n_iter` complete loops of our small subsampled data.
Only 2 iterations are required for convergence on the full dataset of all
chromosomes and strands from\cite{core_analysis_2014}.  Running against the
full dataset using the steps above is explained in Appendix A.
...
model_grocap <- new("TSSHMM")
parameters(model_grocap) <- params_grocap

model_groseq <- new("TSSHMM", bg_proseq = TRUE)
parameters(model_groseq) <- params_groseq
```
...
model_grocap <- new("TSSHMM")
parameters(model_grocap) <- params_grocap
prom_grocap <- granges(viterbi(model_grocap, signal, bg_grocap, tol = 1e-2))
tss_grocap <- tss(signal, prom_grocap)

model_groseq <- new("TSSHMM", bg_proseq = TRUE)
parameters(model_groseq) <- params_groseq
prom_groseq <- granges(viterbi(model_groseq, signal, bg_groseq, tol = 1e-2))
...
(ol <- findOverlaps(tss_grocap, tss_groseq))
length(ol)
length(tss_grocap)
length(tss_groseq)
...
upset(
    fromExpression(
        c(grocap = length(tss_grocap) - length(ol),
          groseq  = length(tss_groseq) - length(ol),
...
length(tss_grocap))` of the GRO-cap reference TSSs.

\bibliography{refs}

\appendix

# Analyzing the full Core 2014 data set

Standalone script to find promoters genome wide, and their transcription start
sites:

```{r Full data run, eval = FALSE}
## Download the script used to generate the example data.
url <- paste(c(repo = "https://gitlab.com/coregenomics/tsshmm",
               sep = "-/raw/master",
               path = "data-raw/core2014.R"), collapse = "/")
file <- basename(url)
...
bg <- core2014full[["GROcap_noTAP"]]

library(tsshmm)
library(futile.logger)
flog.threshold(INFO)

## Split core2014 into batches for training.
system.time(
    batches <- create_batches(signal, bg))

## Save training object.
save(batches, file = "batches_grocap.RData")

## Create the model.
model <- new("TSSHMM")

## Train the model, iterating over full dataset a few times for parameter
## convergence.
system.time({
    i <- 1
    n_iter <- 3
    message_iter <- function()
        message(sprintf("*** Looping entire dataset [%d/%d]", i, n_iter))
    message_iter()
    log <- train(model, batches)
    for (i in 2:n_iter) {
        message_iter()
        to_append <- train(model, batches)
        log <- rbind(log, to_append[-1, ])
    }
})
...
save(params, file = "params_grocap.RData")
save(log, file = "log_grocap.RData")

## Calculate promoter regions and transcription start sites.
prom <- granges(viterbi(model, signal, bg, tol = 1e-2))
pairs <- tss(signal, prom, pairs = TRUE)
```
