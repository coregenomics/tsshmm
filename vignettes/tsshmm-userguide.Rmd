...
system.time({
    ...
    for (i in  2:30) {
        log <- train(model_grocap, batches_grocap)
        log_grocap <- rbind(log_grocap, log[-1, ])
    }
})
...
system.time({
    ...
    for (i in  2:30) {
        log <- train(model_groseq, batches_groseq)
        log_groseq <- rbind(log_groseq, log[-1, ])
    }
})
...
## Visualization of parameters training and convergence

If there are fewer than 1000 rows in total in the batch data, then `train()`
uses a special case algorithm where it runs until convergence.  Such is the
case with our subset example data.  However for a real world mammalian genome
dataset, it's not possible to fit all the data at once into the model;
therefore it becomes necessary to eyeball the training history to decide
whether additional `train()` runs across the full dataset are necessary.  One
can visualize the parameter updates as follows:

```{r Plot training parameters, dependson = paste0("Train GRO-", c("cap", "seq")), cache = params$cache, message = FALSE}
library(dplyr)   # as_tibble, bind_rows, mutate, row_number, %>%
...
df <-
    bind_rows("GRO-cap" = df_long(log_grocap),
              "GRO-seq" = df_long(log_groseq), .id = "bg")
...
ggplot(df, aes(samples, probability, color = parameter, group = parameter)) +
    ...
    labs(title = paste("Model training after 30 iterations",
                       "against different background data types"),
         x = "Number of samples used for training",
         y = "Parameter probability value")
```

As we can see, our parameters converge in 30 iterations on our chr22-
subsampled data.  On the full dataset from\cite{core_analysis_2014},
convergence happens within 2 iterations and of course is not less biased to the
read distribution of the single chromosome because it shuffles the genome wide
training data.
