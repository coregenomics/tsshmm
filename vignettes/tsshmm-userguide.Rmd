...
2. Encode the read coverage counts into the 3 categories of discrete emissions
   ...
   and then
3. Feeding the obs into the model for training using `train()`, and collect the
   collect the `DataFrame` of the parameter changes.
...
## Train the model 30 times to converge parameters for these few observations.
system.time(
    log_grocap <- train(model_grocap, obs_grocap)
)
...
system.time(
    log_groseq <- train(model_groseq, obs_groseq)
)
