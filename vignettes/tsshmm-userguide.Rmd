...
bg <- core2014[["GROcap_noTAP"]]
batches_grocap <- create_batches(signal, bg)
...
plot_params <- function(df)
    ggplot(df, aes(samples, probability, color = parameter, group = parameter)) +
        geom_line() +
        geom_label(data = dplyr::filter(df, i == ceiling(max(i) / 2)),
                   aes(label = parameter)) +
        facet_wrap(~ type) +
        guides(color = "none", label = "none")

plot_params(df_long(log_grocap))
```
...
signal <- core2014[["GROcap_wTAP"]]
bg <- core2014[["GROseq"]]
batches_groseq <- create_batches(signal, bg)
...
plot_params(df_long(log_groseq))
```
...
signal <- core2014[["GROcap_wTAP"]]
bg <- core2014[["GROcap_noTAP"]]
prom_grocap <- granges(viterbi(model_grocap, signal, bg, tol = 1e-2))
...
x <-
    fromExpression(
        c(grocap = length(tss_grocap),
          groseq  = length(tss_groseq),
          `grocap&groseq` = length(ol)))

upset(x, order.by = "freq")
```
